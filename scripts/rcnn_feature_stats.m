...
save_file = sprintf('%s/feature_stats.mat', ...
                    conf.cache_dir);
...
try
  ...
  num_images = min(length(imdb.image_ids), 500);
  boxes_per_image = 200;

  image_ids = imdb.image_ids(randperm(length(imdb.image_ids), num_images));

  ns = [];
  for i = 1:length(image_ids)
    tic_toc_print('feature stats: %d/%d\n', i, length(image_ids));

    d = rcnn_load_cached_pool5_features(imdb.name, image_ids{i});
